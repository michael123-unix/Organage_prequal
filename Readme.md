...
The evaluated models were the [lightGBM](https://lightgbm.readthedocs.io/en/stable/) gradient boosting desition tree, the [ACE](https://github.com/suinleelab/ACE) variational autoencoder developed by the Sun Lee Lab and a simple linear classifier.  
...
The snakmake pipline include hyperparameter optimisaiton for lightGBM and the linear model while for the VAE the default parameters were used, model training and identifying the most informative features as judged by each model. Finally three venn diagrams visualize the overlap of the top 50 features of each model for mice aged 3, 16 and 24 months. 
